<template>
    ...
    <div class="describe-card" :class="{ hide: !now_page_0 }" @click="changePage(false)">
        <div class="des-main" v-if="habitContents && habitContents.length > 0 && habit_index != -1">
            ...
            <div class="des-star">{{ habitContents[habit_index].star_num }}&nbsp;STAR★</div>
        </div>
    ...
    axios.get("/adminapi/user/browsehabits", {
        ...
        if (res.data.code === 401) {
            ...
            for (let i = 0; i < res.data.body.length; i++) {
                let bToday = new Date(today_ymd);
                let bEndDay = new Date(res.data.body[i].habit_end_date);
                if (bToday.getTime() < bEndDay.getTime()) {
                    habitContents.value.push({
                        ...
                        is_complete: res.data.body[i].is_complete, star_num: res.data.body[i].star_num
                    });
                }
